export const ReadonlyNestedList = ({ data, depth = 1 }: { data: any, depth?: number }) => {
    if (Array.isArray(data?.children)) {
        data?.children?.forEach((item: any, index: number) => {
            if (Array.isArray(item?.children?.value)) {
                return <ReadonlyNestedList key={index} data={item?.children} depth={(depth + 1) % 3} />
            }
        })
    }
        ...
        {data?.value?.map((item: any, index: number) => {
            ...
            return (
                <li key={index}>
                    ...
                    {
                        data?.children?.[index]?.children?.value?.length > 0 && <ReadonlyNestedList data={data?.children?.[index]?.children} depth={(depth + 1) as 1 | 2 | 3} />
                    }
                </li>
            );
        })}
